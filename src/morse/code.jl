...
concrete Union) instead of `Vector{AbstractMorseElement}`. The full 5-type union
`MorseElement` is used in `TimedMorseEvent` and dispatch contexts.
"""
...
Convert a single character to its Morse symbol sequence. Returns empty vector for unknown characters."""
...
is_prosign(word::AbstractString) = haskey(PROSIGN_TABLE, uppercase(String(word)))

"""    prosign_to_morse(word::AbstractString) -> Vector{DotOrDash}
Convert a prosign to its Morse symbol sequence."""
prosign_to_morse(word::AbstractString) = get(PROSIGN_TABLE, uppercase(String(word)), DotOrDash[])
